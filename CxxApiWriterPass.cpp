...
static std::string Literally(StringRef);
...
class NameBuilder {
    ...
    std::string getPrefix(Type *Ty) {
        ...
        switch (Ty->getTypeID()) {
            ...
            case Type::IntegerTyID: {
                ...
                if (IntTy->getBitWidth() == 1) return "Bool";
                if (IntTy->getBitWidth() == 8) return "Char";
            ...
            case Type::ArrayTyID: {
                ...
                if (auto IntEleTy = dyn_cast<IntegerType>(EleTy))
                    if (IntEleTy->getBitWidth() == 8) return "String";
    ...
    template <typename T>
    std::string getUnique(T *Obj, const char *Suffix = "") {
        unsigned i = 0;
...
struct CxxApiWriterPass : public ModulePass {
   ...
   private:
    ...
    template <typename T>
    std::string getAggregateElements(const T *);
...
static StringRef getFCmpPred(CmpInst::Predicate P) {
    ...
    assert_cnt(Cmps, CmpInst::FIRST_FCMP_PREDICATE,
...
static StringRef getICmpPred(CmpInst::Predicate P) {
    ...
    assert_cnt(Cmps, CmpInst::FIRST_ICMP_PREDICATE,
...
template <typename T>
static StringRef getCallingConv(const T *Obj) {
...
static std::string Normalize(const std::string &Str) {
    ...
    if (Vec[0] == "llvm") Vec[0] = "LLVM";

    std::string Result;
    for (auto &Ref : Vec) {
        Result += StringRef(Ref.data(), 1).upper() +
                  StringRef(Ref).drop_front().str();
    }
    return Result;
}

static std::string Literally(StringRef Str) {
    std::string Result;
...
/// FIXME: TEST
    ...
    if (((StrVal[0] >= '0' && StrVal[0] <= '9') ||
        ...
        cl() << "BitsToDouble(0x" << utohexstr(BitCast64) << "ULL) /* "
             << StrVal << " */";
    } else {
    ...
    switch (ET->getTypeID()) {
        case Type::IntegerTyID: {
            ...
            switch (BitWidth) {
                ...
                case 64:
                    DefNames.set(
                        PT, "Type::getInt" + utostr(BitWidth) + "PtrTy(Ctx)");
                    return;
                default:
                    DefNames.set(PT, "Type::getIntNPtrTy(Ctx, " +
                                         utostr(BitWidth) + ")");
                    return;
            }
        }
        ...
        default:
            ...
            if (DefNames.has(ET)) {
                ...
                if (AS) {
                    DefNames.set(
                        PT, "PointerType::get(" + EN + ", " + utostr(AS) + ")");
                } else {
...
std::string CxxApiWriterPass::getCDSElements(
...
std::string CxxApiWriterPass::getCDSElementsAsInt(
    const ConstantDataSequential *CDS) {
    std::string Names;
    for (unsigned i = 0; i < CDS->getNumElements(); i++) {
...
#if LLVM_VERSION_BEFORE(3, 9, 1)
            return Arg.getParent()->getAttributes().hasAttribute(
                Arg.getArgNo() + 1, Att.first);
#else
...
void CxxApiWriterPass::printAttributes(const CallInst *CI,
    ...
    std::for_each(Attrs.begin(), Attrs.end(), [&](const Pair &Att) {
        if (CI->getAttributes().hasAttribute(AttributeSet::ReturnIndex,
                                             Att.first)) {
            nl() << Name
                 << "->addAttribute(AttributeSet::ReturnIndex, Attribute::"
                 << Att.second << ");";
        }
    });
#else
    std::for_each(Attrs.begin(), Attrs.end(), [&](const Pair &Att) {
        if (CI->getAttributes().hasAttribute(AttributeList::ReturnIndex,
                                             Att.first)) {
            nl() << Name
                 << "->addAttribute(AttributeList::ReturnIndex, Attribute::"
                 << Att.second << ");";
        }
    });
#endif
...
void CxxApiWriterPass::printStructTypeBody(StructType *ST) {
    if (ST->isOpaque() || ST->isLiteral()) return;
    ...
    switch (Ty->getTypeID()) {
        ...
        case Type::PointerTyID: {
            ...
            if (PT->getAddressSpace()) {
                nl() << "auto " << TypeName << " = PointerType::get("
                     << DefNames.get(ET) << ", " << PT->getAddressSpace()
                     << ");";
            } else {
        ...
        //     printType(ET);
        //     nl() << "auto " << TypeName << " = VectorType::get("
        //          << DefNames.get(ET) << ", " << VT->getNumElements() << ");";
        //     return;
        // }
        default:
...
void CxxApiWriterPass::printTypes(const Module *M) {
    ...
    for (auto &FI : M->functions()) {
        ...
        for (auto &AI : FI.args()) initCommonType(AI.getType(), Ts);
    ...
    for (auto T : Ts)
        if (T->isStructTy()) printStructTypeHead(cast<StructType>(T));
    for (auto T : Ts)
        if (T->isStructTy()) printStructTypeBody(cast<StructType>(T));
    nl();

    for (auto T : Ts)
        if (T->isArrayTy()) printType(T);
    nl();
    for (auto T : Ts)
        if (T->isVectorTy()) printType(T);
    nl();
    for (auto T : Ts)
        if (T->isFunctionTy()) printType(T);
    nl();

    // Init common PointerType again, for better readability
    TypeSet PtrTs;
    for (auto T : Ts)
        if (T->isPointerTy()) initCommonPtrType(cast<PointerType>(T), PtrTs);
...
void CxxApiWriterPass::printConstant(const Constant *C) {
    ...
    if (DefNames.has(C) || isa<GlobalValue>(C)) return;
    ...
    if (auto CI = dyn_cast<ConstantInt>(C)) {
        ...
        OS << CI->getValue();
        OS.flush();
        nl() << "auto " << ValName << " = ConstantInt::get(IRB.getIntNTy("
             << BwStr << "), APInt(" << BwStr << ", \"" << ValStr
             << "\", 10));";
    }
    //
    // ConstantFP
    ...
    // ConstantDataSequential
        ...
        // etc...
            ...
            auto Method = isArrayTy ? " = ConstantArray::get("
                                    : " = ConstantVector::get(";
    ...
    // ConstantExpr
        ...
        if (CE->getOpcode() == Instruction::GetElementPtr) {
            ...
            auto Method = GEP->isInBounds() ? "getInBoundsGetElementPtr"
                                            : "getGetElementPtr";
            ...
            switch (CE->getOpcode()) {
                case Instruction::ICmp:
                    cl() << "ICmp(ICmpInst::ICMP_"
                         << getICmpPred(
                                (ICmpInst::Predicate)CE->getPredicate());
                    break;
                case Instruction::FCmp:
                    cl() << "FCmp(FCmpInst::FCMP_"
                         << getFCmpPred(
                                (FCmpInst::Predicate)CE->getPredicate());
                    break;
...
void CxxApiWriterPass::initCommonConst(const Constant *C, ConstSet &Cs) {
    if (Cs.find(C) != Cs.end()) return;
    Cs.insert(C);

    if (auto BA = dyn_cast<BlockAddress>(C)) {
        DefNames.set(
            C, "BlockAddress::get(" + DefNames.get(BA->getBasicBlock()) + ")");
        return;
    }
    ...
    if (auto CI = dyn_cast<ConstantInt>(C)) {
        SmallString<64> sval;
        CI->getValue().toString(sval, 10, true);
        std::string Val = sval.str().str();
        auto BitWidth = CI->getBitWidth();
        auto BWS = utostr(BitWidth);
        switch (BitWidth) {
            default: {
                if (BitWidth > 64) return;
                DefNames.set(C, "IRB.getIntN(" + BWS + ", " + Val + ")");
                return;
            }
            case 1: {
                DefNames.set(
                    C, CI->isNullValue() ? "IRB.getFalse()" : "IRB.getTrue()");
                return;
            }
            case 8:
            case 16:
...
void CxxApiWriterPass::printConstants(const Module *M) {
    ConstSet Cs;
    for (auto &G : M->globals())
        if (G.hasInitializer()) initCommonConst(G.getInitializer(), Cs);

    for (auto &F : M->functions()) {
        if (F.hasPersonalityFn()) initCommonConst(F.getPersonalityFn(), Cs);
        for (auto I = inst_begin(F), E = inst_end(F); I != E; ++I)
            for (auto &Op : I->operands())
                if (auto C = dyn_cast<Constant>(&Op)) initCommonConst(C, Cs);
    }
    ...
    for (auto C : Cs)
        if (!isa<ConstantExpr>(C)) printConstant(C);
    for (auto C : Cs) printConstant(C);
}
...
std::string CxxApiWriterPass::getOperandName(const Value *V, ValueMap &Refs) {
    ...
    if (Refs.find(V) != Refs.end()) return Refs[V];

    if (!isa<Instruction>(V) || DefNames.has(V)) return DefNames.get(V);
...
bool CxxApiWriterPass::isCommonIntrinsic(const CallInst *CI) {
    if (auto F = CI->getCalledFunction()) return isCommonIntrinsic(F);
    return false;
}
...
void CxxApiWriterPass::printIntrinsic(const CallInst *CI, ValueMap &Refs) {
    ...
    switch (F->getIntrinsicID()) {
        case Intrinsic::lifetime_start: {
            nl() << "IRB.CreateLifetimeStart(" << ArgNames[1] << ", "
                 << ArgNames[0] << ");";
            return;
        }
        case Intrinsic::lifetime_end: {
            nl() << "IRB.CreateLifetimeEnd(" << ArgNames[1] << ", "
                 << ArgNames[0] << ");";
            return;
        }
        case Intrinsic::memcpy: {
            ...
            auto Volatile = btostr(
                cast<ConstantInt>(CI->getArgOperand(4))->getZExtValue() != 0);
...
void CxxApiWriterPass::printInstruction(const Instruction *I, ValueMap &Refs) {
    ...
    switch (I->getOpcode()) {
        ...
        case Instruction::IndirectBr: {
            ...
            nl() << "auto " << ValName << " = IRB.CreateIndirectBr("
                 << OpNames[0] << ", " << NumDest << ");";
        ...
        case Instruction::Invoke: {
            ...
            nl() << "auto " << ValName << " = IRB.CreateInvoke("
                 << getOperandName(Inv->getCalledOperand(), Refs) << ", "
        ...
        case Instruction::Load: {
            ...
            if (LD->getAlignment()) {
                ...
                cl() << "IRB.CreateLoad(" << OpNames[0] << Vol
                     << lastNameArg(I);
            }
            if (LD->isAtomic()) {
        ...
        case Instruction::Store: {
            ...
            if (ST->getAlignment()) {
                cl() << "IRB.CreateAlignedStore(" << OpNames[0] << ", "
                     << OpNames[1] << ", " << ST->getAlignment()
        ...
        case Instruction::GetElementPtr: {
            ...
            auto Method =
                GEP->isInBounds() ? "CreateInBoundsGEP(" : "CreateGEP(";
        ...
        case Instruction::Call: {
            ...
            auto FunName = getOperandName(Call->getCalledOperand(), Refs);
            if (auto ILA = dyn_cast<InlineAsm>(Call->getCalledOperand())) {
                FunName = "ILA" + ValName;
        ...
        case Instruction::ExtractElement: {
            nl() << "auto " << ValName << " = IRB.CreateExtractElement("
                 << OpNames[0] << ", " << OpNames[1] << lastNameArg(I);
            return;
        }
        case Instruction::InsertElement: {
            nl() << "auto " << ValName << " = IRB.CreateInsertElement("
                 << OpNames[0] << ", " << OpNames[1] << ", " << OpNames[2]
                 << lastNameArg(I);
            return;
        }
        case Instruction::ShuffleVector: {
            nl() << "auto " << ValName << " = IRB.CreateShuffleVector("
                 << OpNames[0] << ", " << OpNames[1] << ", " << OpNames[2]
                 << lastNameArg(I);
            return;
        }
        case Instruction::ExtractValue: {
            ...
            for (auto i : EVI->indices()) Idxs += utostr(i) + ", ";
            Idxs = StringRef(Idxs).drop_back(2).str() + "}";
            cl() << "auto " << ValName << " = IRB.CreateExtractValue("
                 << OpNames[0] << ", " << Idxs << lastNameArg(I);
            return;
        }
        case Instruction::InsertValue: {
            auto IVI = cast<InsertValueInst>(I);
            auto Idxs = std::string("{");
            for (auto i : IVI->indices()) Idxs += utostr(i) + ", ";
            Idxs = StringRef(Idxs).drop_back(2).str() + "}";
            cl() << "auto " << ValName << " = IRB.CreateInsertValue("
                 << OpNames[0] << ", " << OpNames[1] << ", " << Idxs
                 << lastNameArg(I);
            return;
        }
        ...
        case Instruction::LandingPad: {
            ...
            nl() << "auto " << ValName << " = IRB.CreateLandingPad("
                 << ValTyName << ", " << LPI->getNumClauses() << lastNameArg(I);
            if (LPI->isCleanup()) nl() << ValName << "->setCleanup(true);";
        ...
        case Instruction::AtomicCmpXchg: {
            ...
            nl() << "auto " << ValName << " = IRB.CreateAtomicCmpXchg("
                 << OpNames[0] << ", " << OpNames[1] << ", " << OpNames[2]
                 << ", " << SuccOrd << ", " << FailOrd << ", " << SyncScope
                 << ");";
            if (I->hasName())
                nl() << ValName << "->setName(" << Literally(I->getName())
                     << ");";
            if (CXI->isVolatile()) nl() << ValName << "->setVolatile(true);";
            if (CXI->isWeak()) nl() << ValName << "->setWeak(true);";
            nl();
            return;
        }
        case Instruction::AtomicRMW: {
            ...
            nl() << "auto " << ValName
                 << " = IRB.CreateAtomicRMW(AtomicRMWInst::" << Operation
                 << OpNames[0] << ", " << OpNames[1] << ", " << Ordering << ", "
                 << SyncScope << ");";
            if (I->hasName())
                nl() << ValName << "->setName(" << Literally(I->getName())
                     << ");";
            if (RMWI->isVolatile()) nl() << ValName << "->setVolatile(true);";
            nl();
            return;
        }
        case Instruction::AddrSpaceCast: {
            nl() << "auto " << ValName << "IRB.CreateAddrSpaceCast("
                 << OpNames[0] << ", " << ValTyName << lastNameArg(I);
            return;
        }
...
void CxxApiWriterPass::printFunctionHead(const Function *F) {
    ...
    if (F->isDeclaration()) {
        ...
        if (hasAnyArgAttribute(F)) cl() << ", has arg attrs";
        if (F->hasPersonalityFn()) cl() << ", has personalityfn";
    }
...
void CxxApiWriterPass::printFunctionAttr(const Function *F) {
    ...
    if (F->hasPersonalityFn()) {
        nl() << Name << "->setPersonalityFn("
             << DefNames.get(F->getPersonalityFn()) << ");";
    }
    ...
    if (!F->arg_empty()) {
        nl() << "SmallVector<Argument *, "
             << F->getFunctionType()->getNumParams() << "> Args;";
...
void CxxApiWriterPass::printFunctionBody(const Function *F) {
    ...
    for (auto &BI : F->getBasicBlockList()) {
        ...
        for (auto &I : BI.getInstList()) printInstruction(&I, ForwardRefs);
    }
...
void CxxApiWriterPass::printModuleBody() {
    ...
    if (!TheModule->empty()) {
        ...
        for (auto &F : TheModule->functions())
            if (!isCommonIntrinsic(&F)) printFunctionHead(&F);
    }
    ...
    if (!TheModule->getGlobalList().empty()) {
        ...
        nl() << "//";
        for (auto &G : TheModule->globals()) printVariableHead(&G);
    }
    ...
    if (!TheModule->getGlobalList().empty()) {
        ...
        nl() << "//";
        for (auto &G : TheModule->globals()) printVariableBody(&G);
    }
...
bool CxxApiWriterPass::runOnModule(Module &M) {
    ...
    indent();
    nl() << "auto M = std::make_unique<Module>("
